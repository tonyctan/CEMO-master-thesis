...
finlit <- pisa.select.merge(
    ...
    countries = c(
        "BRA", "CHL", "ESP", "EST", "GEO", "IDN", "ITA", "LTU", "PER", "POL", "PRT", "SVK", "USA"
    )
)
...
finlit <- cbind(finlit[, c(1:35)], MALE, IMMI1GEN, IMMI2GEN, finlit[, c(38:51)])

# Throw away cases whose school weights (col #48) are NA
