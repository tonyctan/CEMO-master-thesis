...
Iterations whose model fit indices fell short of the abovementioned cutoff criteria were investigated using modification indices and standardised expected parameter change. Modification indices (MI) suggest how much a model's $\chi^2$ statistic would decrease should a fixed parameter were freely estimated; modification indices greater than $3.84$ (cirtical value of $\chi^2_1$ at $\alpha=.05$) warrent further consideration for theoretical plausibility \parencite{whittaker:2012}. The expected parameter change (EPC), in contrast, indicates the estimate value of a fixed parameter if it were added to a model and freely estimated, providing a more direct estimate of the size of the misspecification for the parameters under consideration. This study made use of the decision rule prescribed by \textcite{saris:1987} to freely estimate a parameter if it received both large MI and large EPC.
