...
In order to separate the incremental effect attributable to school-level variables, a student-level only model was first established as a reference (``One-level model'' in \cref{tab:est1}). Even with student-only variables, model fit indices $\text{CFI} = .97$, $\text{TLI} = 0.927$ and $\text{SRMR} = .016$ jointly suggested the proposed input (school climate)--mediator (affect)--output (financial literacy cognitive outcome) model was a meaningful one.

School-level variables then allowed to covary amongst other on top of the student-only structure, forming the two-level saturated model. This procedure had an effect of decomposing total residual variances into student- and school-levels. Resultantly, $L1$ residual variance reduced by more than a quater, from 7,866 to 5,764, indicating the necessity of a $L2$ structure.

\section{Full Model}

Relationship amongst school-level variables were further imposed at $L2$, transforming the saturated model into the final SEM model as illustrated in \cref{fig:model}.
...
All three student-level school climate variables shared statistically significant relationships with financial literacy cognitive outcomes. School safety (\texttt{NOBULLY}) was positively correlated with financial literacy both directly and through affective mediators.


%Counter intuitively, the associations were \emph{negative} between financial literacy and both school and family efforts, a result more likely to be resulted from compensation than from causation.
