...
% Set demographic vars
    \node[demo] (escs) at (0,4) {$\texttt{SES}$};
    \node[demo] (immi) at (0,2) {$\texttt{Immigration}$};
    \node[demo] (male) at (0,0) {$\texttt{Being male}$};

% Set school climate vars (X)
    \node[manvar] (fedu) at (0,10) {$\texttt{F education}$};
    \node[manvar] (fsoc) at (0,8) {$\texttt{F socialisation}$};
    \node[manvar] (safe) at (0,6) {$\texttt{Safety}$};

% Set outcome var (M and Y)
    \node[manvar] (fc) at (7.5,10) {$\texttt{F knowledge}$};
    \node[manvar] (fb) at (7.5,5) {$\texttt{F behaviour}$};
    \node[manvar] (fa) at (7.5,0) {$\texttt{F confidence}$};

% Set school-level var
    \node[manvar] (ssh) at (13,8) {$\texttt{Edu shortage}$};
    \node[demo] (sst) at (13,2) {$\texttt{Class size}$};
...
% Link demo var to F outcomes
    \draw[black!25!white,<->] (escs.east) to node[above,pos=0.35] {H14 $>0$} (3,4);
    \draw[black!25!white,<->] (immi.east) to node[above] {H15 $<0$} (3,2);
    \draw[black!25!white,<->] (male.east) to node[above] {H16 $>0$} (3,0);
