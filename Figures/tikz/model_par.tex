...
% Link mediators with outcome var
    \draw[blue,->] (fami1.east) to node[above] {$\beta_1$} (flit1.west);
    \draw[blue,->] (conf1.east) to node[below] {$\beta_2$} (flit1.west);
...
% Link demographic vars with outcome var
    \draw[gray,->] (escs.east)--(fami1.west);
    \draw[gray,->] (escs.east)--(conf1.west);
    \draw[gray,->] (escs.east)--(flit1.west);

    \draw[gray,->] (immi2.east)--(fami1.west);
    \draw[gray,->] (immi2.east)--(conf1.west);

    \draw[gray,->] (immi1.east)--(flit1.west);

    \draw[gray,->] (male.east)--(fami1.west);
    \draw[gray,->] (male.east)--(conf1.west);
...
% Arc covariances
    ...
    \draw[gray,<->] (escs.west) to [bend right] (A1.west);
    \draw[gray,<->] (escs.west) to [bend right] (C1.west);
    \draw[gray,<->] (escs.west) to [bend left] (immi2.west);
    \draw[gray,<->] (escs.west) to [bend left] (immi1.west);
...
% Link input vars with output var
    ...
    \draw[->] (C2.east) to node[below,pos=0.4] {$a_2$} (flit2.west);
    \draw[->] (S2.east) to node[above,pos=0.4] {$a_3$} (flit2.west);
...
\end{tikzpicture}
}{This colouring convention applies to the student-level path diagram to enhance readability: Exogenous and endogenous variables are boxed in red and blue respectively while control variables receive gray background colour. The direct and indirect paths of the structural component are represented in red and blue arrows respectively. Gray double-headed arcs represent covariances between variables.}
