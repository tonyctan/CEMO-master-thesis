...
% Lat var "Academic"
    \node[latvar] (A1) at (0,0) {$\texttt{PERFEED}_B$};
    \node[latvar] (A2) at (0,-1) {$\texttt{TEACHINT}_B$};
    \node[latvar] (A3) at (0,-2) {$\texttt{FLSCHOOL}_B$};

    \node[latvar] (A) at (5,-1) {$\texttt{Academic}_B$};
    % Variance arrow
...
% Lat var "Safety"
    \node[latvar] (S1) at (0,-3) {$\texttt{BELONG}_B$};
    \node[latvar] (S2) at (0,-4) {$\texttt{BULLY}_B$};

    \node[latvar] (S) at (5,-3.5) {\texttt{Safety}$_B$};
    % Variance arrow
...
% Financial socialisation
    \node[latvar] (C) at (5,-6) {$\texttt{FLFAMILY}_B$};
...
% Outcome variable
    \node[latvar] (finlit) at (10,-4.75) {$\texttt{FLIT}_{B2}$};

    \filldraw [black] (8.25,-4.75) circle (2pt);
